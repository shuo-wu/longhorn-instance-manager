...
func (r *Replica) construct(bdevLvolMap map[string]*spdktypes.BdevInfo) (err error) {
	...
	switch r.State {
	case types.InstanceStatePending, types.InstanceStateStopped:
		break
	...
	if len(r.ActiveChain) > 1 {
		...
		for lvolName, bdevLvol := range bdevLvolMap {
			if bdevLvol.DriverSpecific.Lvol.BaseSnapshot != "" {
				...
				if r.ActiveChain[0] != nil && r.ActiveChain[0].Name == lvolName {
...
func (r *Replica) rebuildingSrcShallowCopyStatusUpdateAndHandlingNoLock(spdkClient *spdkclient.Client) (status *spdktypes.ShallowCopyStatus, err error) {
	...
	if status, err = spdkClient.BdevLvolCheckShallowCopy(r.rebuildingSrcCache.shallowCopyOpID); err != nil {
		return nil, err
	}
