...
func (r *Replica) SnapshotHash(spdkClient *spdkclient.Client, snapshotName string, rehash bool) (err error) {
	...
	if !rehash && snapSvcLvol.SnapshotChecksum != "" {
		return fmt.Errorf("snapshot %s(%s)(%s) already has checksum %s while rehash is disabled", snapshotName, snapLvolName, snapSvcLvol.UUID, snapSvcLvol.SnapshotChecksum)
	}
