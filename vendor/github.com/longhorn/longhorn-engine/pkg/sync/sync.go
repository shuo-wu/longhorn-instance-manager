...
func (t *Task) AddReplica(volumeSize, volumeCurrentSize int64, address, instanceName string, fileSyncHTTPClientTimeout int, fastSync bool, localSync *types.FileLocalSync, grpcTimeoutSeconds int64) error {
	...
	logrus.Infof("Testing................. adding replica %s", address)
	...
	logrus.Infof("Adding replica %s from existing replicas %+v", address, fromAddressMap)
	...
	logrus.Infof("Prepared rebuild for replica %s, got files %+v to sync", address, resp)
...
func (t *Task) getTransferClients(address, instanceName string) (*replicaClient.ReplicaClient, map[string]bool, error) {
	var err error
